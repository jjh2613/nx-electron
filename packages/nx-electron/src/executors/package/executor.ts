...
function _createBaseConfig(
  ...
  const renderFiles = files.concat([
    './package.json',
    {
  ...
  if (options.subFrontendProject) {
    renderFiles.push({
      from: resolve(options.sourcePath, options.subFrontendProject),
      to: options.subFrontendProject,
      filter: ['**/!(*.+(js|css).map)', 'assets'],
    });
  }
...
function _createConfigFromOptions(
  ...
  delete config.subFrontendProject;
